# Legacy-good units not reproduced by rescue: no confirmed loss, five unresolved (Luke0804 imec0)
...
> circular-shift nulls. The cohort is now **137**. **No unit was positively
> classified absent at detection or removed by curation; 132/137 have supported
> overlap and five remain unresolved.** This is no confirmed loss, not proof
> that no neuron was lost.
...
**Answer:** 132/137 have null-controlled evidence of overlap with rescue
detections. Five are `detection status unresolved` — low-amplitude (11 µV),
high-rate (3.6 Hz) units this statistic cannot adjudicate. Their other quality
metrics do not resolve whether rescue detected them.
...
## Result: no confirmed loss; five unresolved
...
**Checkpoint A's “substantial confirmed loss” trigger did not fire.** The audit
supports preservation/repartitioning for 132 units and leaves five unresolved.
Their low amplitude and high refractory burden make them marginal legacy units,
but do not prove that rescue detected them. No detection or curation-drop
regression is confirmed on this probe.
...
For the supported cases, the rescue configuration applies a **bidirectional
curation threshold shift** —
...
small clusters in both directions. This describes where the output difference
appears; it does not rule out preprocessing or motion representation as an
upstream cause. See
...
- **89 dispersed.** Low rate (median 0.17 Hz), the worst refractory of any
  ...
  spike pool; this does not establish biological identity or exclude moving
  neurons.
...
- **5 detection status unresolved.** 11 µV, 3.6 Hz, 40% refractory violation —
  the coincidence statistic cannot place them and the null test does not clear.
  They are marginal legacy units, but their detection status remains unknown.

## Consequences

1. **No substantial regression is confirmed in the −137.** Phase A's stated trigger —
   "a substantial share are genuine neurons lost at detection or curation" — did
   not fire. Five cases remain unresolved and cannot be used as evidence of
   either loss or preservation.
