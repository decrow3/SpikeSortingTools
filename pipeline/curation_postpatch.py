...
def _resolve_merge_groups(raw_merge_groups):
    ...
    Input format matches SpikeInterface: list[list[unit_id]].
    Returns list[list[unit_id]] where overlapping groups are merged.
    """
...
def run_cur(
    ...
    temporal_presence_distance_thresh=100,
    enable_ccg_similarity=True,
    ccg_corr_diff_thresh=0.25,
):
    '''
    ...
    if npy_path.exists() and not recalc:
        ...
        raw_merge_list = []
        if enable_temporal_splits:
            merges_temporal = compute_merge_unit_groups(
                analyzer,
                preset="temporal_splits",
                presence_distance={"presence_distance_thresh": float(temporal_presence_distance_thresh)},
            )
            print(f"Found {len(merges_temporal)} temporal merges.")
            raw_merge_list += merges_temporal

        if enable_ccg_similarity:
            merges_ccg = compute_merge_unit_groups(
                analyzer,
                preset="similarity_correlograms",
                correlogram={"corr_diff_thresh": float(ccg_corr_diff_thresh)},
            )
            print(f"Found {len(merges_ccg)} CCG/Similarity merges.")
            raw_merge_list += merges_ccg

        merge_unit_groups = _resolve_merge_groups(raw_merge_list)
        print(f"Final resolved merge groups: {len(merge_unit_groups)}")
    ...
    # append new ids as a numpy array; works even when n_groups == 0
