...
OUT=ROOT/'testing/outputs/luke_population_depth_v2'
...
def main():
 ...
 if not valid('templates'):
  ts=np.load(BASE/'cur/cur_output/spike_times.npy',mmap_mode='r').ravel();ids=np.load(BASE/'cur/cur_output/spike_clusters.npy',mmap_mode='r').ravel();proposals={};records=[];seedframes={}
  for start in [930,980]:
   ...
   for cid in np.unique(cc):
    ...
    records.append(r)
    if start==930:proposals[cid]=(r,w,noise);seedframes[cid]=ev+first
   del x
 ...
 for r in sel.itertuples():
  ...
  # Verify signed exact geometry translations independently of recording/DREDGE.
  checks=[]
  for dest,shift in zip(maps,sh):
   assert np.all(geo[dest,1]-geo[ch,1]==shift) and np.all(geo[dest,0]==geo[ch,0])
   synthetic=np.zeros((101,384),dtype=np.float32);synthetic[20:81,dest]=v
   planted=np.array([scores(synthetic,[50],dc,v,weights)[0,0] for dc in maps]);assert sh[int(planted.argmax())]==shift
   checks.append(dict(shift_um=int(shift),recovered_um=int(sh[int(planted.argmax())])))
  atomic_json(OUT/f'geometry_unit_{cid}.json',checks)
  # Conservative shape prefilter, max over ±6 relative samples; not identity proof.
  rivals=[]
  for dest in maps:
   rr=[]
   for q in inv.itertuples():
    ...
    for s in SHIFTS:
     ...
     for lag in range(-6,7):
      shifted=np.zeros_like(rv)
      if lag<0:shifted[:lag]=rv[-lag:]
      elif lag>0:shifted[lag:]=rv[:-lag]
      else:shifted=rv
      sims.append(float(np.sum(weights*v*shifted)/np.sqrt(np.sum(weights*v*v)*np.sum(weights*shifted*shifted)+1e-20)))
     if max(sims)>.35:rr.append((q.unit_id,int(s),rv))
   rivals.append(rr)
 ...
 for start in range(930,1030,10):
  ...
  partial=list(OUT.glob(stage+'_*'))
  if partial:
   archive=OUT/('interrupted_'+stage+'_'+str(time.time_ns()));archive.mkdir()
   for pp in partial:pp.rename(archive/pp.name)
  ...
  for c in models:
   ...
   status=np.full(len(ev),'unmatched',dtype='<U24');ii=np.flatnonzero(best>=.86);status[ii]='accepted';status[ii[best[ii]-second[ii]<.025]]='depth_ambiguous';status[ii[best[ii]-rs[ii]<.025]]='identity_ambiguous';boundary=(c['sh'][win]==c['sh'].min())|(c['sh'][win]==c['sh'].max());status[ii[boundary[ii]& (status[ii]=='accepted')]]='boundary'
   # Duplicate detections collapse per identity, not across independent units.
   keep=[];occupied=np.zeros(len(x),bool)
   priority=np.where(np.isin(status,['accepted','depth_ambiguous','boundary']),0,1)
   for i in np.lexsort((-best,priority)):
    if best[i]<.86:continue
   ...
   for k,i in enumerate(keep):rows.append(dict(unit_id=cid,frame=int(fr[k]+first),time_s=(fr[k]+first)/fs,shift_um=int(c['sh'][win[i]]),centroid_um=cent[k],score=best[i],gain=sc[win[i],i,1],identity_margin=best[i]-rs[i],shift_margin=best[i]-second[i],rival_id=int(rid[i]),status=status[i],search_boundary=bool(boundary[i]),training_overlap=930<=start<940))
