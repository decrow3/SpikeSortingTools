...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--method',choices=['dredge','medicine'],required=True);ap.add_argument('--input',required=True);ap.add_argument('--stage',type=Path,required=True);args=ap.parse_args();stage=args.stage;source=SRC/'luke_early_sigma_screen250_v1/fields'/args.input;mp=Path('/mnt/NPX/Luke/20250804/rescue_pipeline_results_Luke0804_V2V1_g0_imec0/recording/rescue_recording_manifest.json');m=json.loads(mp.read_text());fs=m['sampling_frequency_hz'];p=np.load(source/'peaks.npy');loc=np.load(source/'locations.npy');torch.set_num_threads(4);start=time.monotonic();np.random.seed(0);torch.manual_seed(0)
