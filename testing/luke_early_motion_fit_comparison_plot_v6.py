...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';OUT=SRC/'luke_early_medicine_peak_comparison_v6';OLD=SRC/'luke_early_medicine_peak_comparison_v1'
...
for col,arm in enumerate(['original','compensated']):
 ...
 for name,src,c in [('MEDiCINe · internal ±250 µm',OUT,'#D55E00')]:
...
fig.suptitle('DREDGE versus MEDiCINe · 0.25 s time bins · nominal 1 s smoothing · same peaks · full displacement range\nAll traces referenced to their median at 970–975 s; shared vertical scale within each row')
