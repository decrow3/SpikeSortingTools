...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';OUT=SRC/'luke_ap_methods_sweep_v1';SCREENS=['none','center_only','relaxed','full']
  ...
  with PdfPages(OUT/f'01_{method}_all17_sweep.pdf') as pdf:
   for cell in c.itertuples():
    ...
    for ax,screen in zip(axs.flat,SCREENS):
     observations(ax,e[e.unit_id==cell.unit_id])
     for sigma,color in COLORS.items():t,v=curves[(f'{sigma}sigma_{screen}_bound250',method,cell.unit_id)];ax.plot(t,v,c=color,lw=1,label=f'{sigma}σ')
     ax.set_title(screen.replace('_',' '));ax.legend(fontsize=8,ncol=3)
    fig.suptitle(f'{method.upper()} · unit {cell.unit_id} · seed {cell.seed_centroid_um:.0f}µm · 930–1030s',fontsize=16);fig.supxlabel('Colored:method curves; black filled:strict waveform; gray open:lower-score; gray crosses:identity ambiguous. Shading:seed interval.\nSame cached input arrays per combination and frozen17 candidates. Common seed-event reference; no matching to field or smoothed waveform path.',fontsize=10);pdf.savefig(fig)
    if cell.unit_id in [161,555,657,673]:fig.savefig(OUT/f'{method}_unit_{cell.unit_id}.png',dpi=140)
    plt.close(fig)
 with PdfPages(OUT/'02_head_to_head_5sigma_8sigma.pdf') as pdf:
  for cell in c.itertuples():
   fig,axs=plt.subplots(2,2,figsize=(17,12),sharex=True,sharey=True,layout='constrained')
   for ax,screen in zip(axs.flat,SCREENS):
    observations(ax,e[e.unit_id==cell.unit_id])
    for method,color in [('dredge','#0072B2'),('medicine','#CC79A7')]:
     for sigma,ls in [(5,'-'),(8,'--')]:t,v=curves[(f'{sigma}sigma_{screen}_bound250',method,cell.unit_id)];ax.plot(t,v,c=color,ls=ls,lw=1.2,label=f'{method} {sigma}σ')
    ax.set_title(screen.replace('_',' '));ax.legend(fontsize=8,ncol=2)
   fig.suptitle(f'Documented DREDGE/MEDiCINe · fixed5σ and8σ examples · unit {cell.unit_id}',fontsize=16);fig.supxlabel('5σ and8σ chosen for display before results,not selected winners. Full five-threshold sweeps are in the separate method PDFs.\nBlack/gray markers preserve all waveform evidence classes; models compared on common event-time support,without fitting to lighthouses.',fontsize=10);pdf.savefig(fig);plt.close(fig)
 (OUT/'README.md').write_text('''# Documented DREDGEv8 and MEDiCINev6 across the full early sweep\n\n40 combinations:2 methods ×5 detection thresholds(3,4,5,6,8σ) ×4 screens(none,center-only,relaxed,full). Exact peak/location arrays from the completed250µm sweep are reused. Cached5σ/no-screen fits are reused only after exact array equality. Remaining38 fits are fresh.\n\nOpen00_cached5sigma_lighthouse_comparison.pdf for the initial baseline test;01_dredge_all17_sweep.pdf and01_medicine_all17_sweep.pdf for the full method sweeps;02_head_to_head_5sigma_8sigma.pdf for predeclared5/8σ comparisons. These are not selected winners.\n\nDREDGEv8:0.25s bins,0.5s histogram Gaussian sigma,20 windows,strict±250µm pairwise bound. MEDiCINev6:0.25s bins,1s triangular kernel,4 depth bins,motion_bound500,10000training steps,seed0,CUDA,all supplied absolute amplitudes retained. Temporal kernels and spatial bases are not equivalent. MEDiCINe's internal range differs in meaning from DREDGE's pairwise bound.\n\nThe17 candidate identities and strict/lower-score/ambiguous classes stay frozen. Fields are evaluated at fixed seed centroids; offsets use identical seed-spike timestamps in common field support. Event differences use identical temporal intersection across all40 fields and no extrapolation. Report all observations and|waveform displacement|≥120µm separately; require≥5 events for candidate/class/scope summaries. Median across candidate medians is descriptive,not an independent-cell confidence interval or motion accuracy ranking. No automatic production adoption.\n\nAll input hashes,resolved settings,per-arm commands,stdout/stderr,exit receipts,fit audits and completed-arm checksums are saved. Interrupted optimizer fits cannot resume within an arm; preserve evidence and investigate before restarting.\n''');(OUT/'report_validation.json').write_text(json.dumps(dict(status='complete',combinations=40,candidates=17,common_time_support_s=[float(lo),float(hi)],no_spatial_extrapolation=True,reference_events_common=True),indent=2));print('Full method reports complete',flush=True)
