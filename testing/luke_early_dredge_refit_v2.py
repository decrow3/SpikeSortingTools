...
def main():
    ...
    cfg=json.loads((PRE/'settings.json').read_text())['estimator']
    (OUT/'dredge_settings.json').write_text(json.dumps(dict(estimator=cfg,strict_pairwise_bound_um=80,scope='Fresh fit on exact 930–1030 cached peaks, all settings unchanged',source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()),indent=2))
    ...
    for arm in ['original','compensated']:
        ...
        delta=float(np.max(np.abs(motion.displacement[0]-np.load(PRE/f'{arm}_dredge_motion.npz')['displacement_um'])))
