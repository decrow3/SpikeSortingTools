...
OUT=ROOT/'testing/outputs/luke_stationary_stage_audit_v2'
def main():
 ...
 stages=list(zip(['acquisition','phase','blanked','interpolated'],reversed(chain)));stages=[v for v in stages if v[0]=='interpolated'];(OUT/'resolved_provenance.json').write_text(json.dumps(prov,indent=2));events=pd.read_csv(ROOT/'testing/outputs/luke_stationary_voltage_audit_v1/matched_events.csv',dtype={'source':str});e=np.sort(events[events.source=='251_252'].frame.to_numpy())-first;e=e[np.linspace(0,len(e)-1,300,dtype=int)];off=np.arange(-90,91);tt=off/fs*1000;sos=butter(3,[300,6000],fs=fs,btype='bandpass',output='sos');payload={};rows=[json.loads(line) for line in (ROOT/'testing/outputs/luke_stationary_stage_audit_v1.service.log').read_text().splitlines() if line.startswith('{"stage":')];phase_snapshot=None;blank_snapshot=None
 ...
 for name,d in stages+[('cached',None)]:
  if d is None:a=cache
   ...
   r=si.load_extractor(d,base_folder=BASE/'recording');assert r.get_num_channels()==384 and r.get_sampling_frequency()==fs;a=r.get_traces(start_frame=first-pad,end_frame=first+n+pad)
 ...
 fig,axes=plt.subplots(2,3,figsize=(15,8),sharex=True);names=['interpolated','cached']
 for name in names:
