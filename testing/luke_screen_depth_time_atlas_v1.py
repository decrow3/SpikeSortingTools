"""Matched peak depth/time histograms for all saved screen masks; cached arrays only."""
...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';SWEEP=SRC/'luke_screen_sweep_v1';OUT=SRC/'luke_screen_depth_time_atlas_v1'
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
def main():
 ...
 t=4160+peaks['sample_index']/fs;z=loc['y'];amp=abs(peaks['amplitude']);te=np.arange(4160,4261,1.);de=np.arange(0,3841,10.)
 ...
 base=histogram(np.ones(len(peaks),bool));vmax=[float(np.log1p(a).max()) for a in base]
 rows=[];sources=[pp,yp,mp,meta,Path(__file__).resolve()];index=['# Screening peak depth/time atlas','','[Complete 21-page PDF](all_21_depth_time.pdf)','','Every page shows the same baseline, retained peaks, and removed peaks: count at left, absolute detected-amplitude sum at right. Localized depth; 1 s × 10 µm bins. Cividis colors show log(1 + bin total), with identical limits across rows and variants for each metric. No per-arm normalization or quantile clipping. Off-probe localizations are counted explicitly in the caption. These are detected populations, not sorted units.','','| Page | Variant | Retained | Depth/time PNG | PDF | Nine-cell overlay |','|---|---|---:|---|---|---|']
 with PdfPages(OUT/'all_21_depth_time.pdf') as pdf:
  for page,r in enumerate(manifest.itertuples(),1):
   ...
   retained=histogram(keep);removed=histogram(~keep)
   ...
   fig,axes=plt.subplots(3,2,figsize=(15,11),sharex=True,sharey=True,layout='constrained')
   for ri,(title,data) in enumerate([('Unscreened compensated 5σ baseline',base),('Retained',retained),('Removed',removed)]):
    for col in range(2):
     ax=axes[ri,col];im=ax.imshow(np.log1p(data[col]),origin='lower',aspect='auto',extent=[te[0],te[-1],de[0],de[-1]],interpolation='nearest',cmap='cividis',vmin=0,vmax=vmax[col]);ax.set(title=title+' · '+('peak count' if col==0 else 'amplitude sum'),ylabel='Localized depth (µm)',xlabel='Recording time (s)');ax.ticklabel_format(axis='x',style='plain',useOffset=False)
     if ri==2:fig.colorbar(im,ax=axes[:,col],shrink=.7,label='log(1 + '+('count)' if col==0 else 'summed |peak amplitude| in µV)'))
   ...
   fig.supxlabel(f'Shared color limits across all 21 variants; 1 s × 10 µm bins. {outside:,} retained events outside plotted time/depth bounds.\nColor represents detected population density or amplitude mass, not biological identity or motion accuracy.',fontsize=10)
 ...
 (OUT/'provenance.json').write_text(json.dumps(dict(source_sha256={str(p):sha(p) for p in sources},render_only=True,bin_s=1,bin_um=10,color='cividis',shared_log1p_vmax=vmax,partition_checks_pass=True),indent=2))
