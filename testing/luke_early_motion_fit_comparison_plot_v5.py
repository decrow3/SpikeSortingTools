...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';OUT=SRC/'luke_early_medicine_peak_comparison_v5';OLD=SRC/'luke_early_medicine_peak_comparison_v1'
...
for col,arm in enumerate(['original','compensated']):
 ...
 for name,src,c in [('MEDiCINe 1 s time bins',SRC/'luke_early_medicine_peak_comparison_v4','#D55E00'),('MEDiCINe 0.25 s time bins',OUT,'#009E73')]:
...
fig.suptitle('MEDiCINe 1 s kernel, 4 depth bins: temporal resolution comparison · same peaks · full displacement range\nAll traces referenced to their median at 970–975 s; shared vertical scale within each row')
