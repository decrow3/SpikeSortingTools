...
OUT=SRC/'luke_early_medicine_peak_comparison_v5'
PRE=SRC/'luke_long_context_validation_v1'
FS=29999.835983263598
ARGS=dict(time_bin_size=.25,num_depth_bins=4,time_kernel_width=1.,amplitude_threshold_quantile=0.,training_steps=10000,plot_figures=False)
def main():
    ...
    for col,arm in enumerate(['original','compensated']):
        z=np.load(SRC/'luke_early_medicine_peak_comparison_v2'/f'{arm}_dredge_motion.npz')
        methods=[('DREDGE fresh fit',z['time_s'],z['depth_um'],z['displacement_um'],'#56E0E8'),('MEDiCINe · 1 s kernel · 0.25 s time bins · 4 depth bins',np.load(OUT/arm/'time_bins.npy'),np.load(OUT/arm/'depth_bins.npy'),np.load(OUT/arm/'motion.npy'),'#FFFFFF')]
    ...
    fig.suptitle('Same detected peaks · fresh DREDGE / MEDiCINe with 1 s kernel · 0.25 s time bins · 4 depth bins\nCurves anchored by median displacement at 970–975 s; offsets are illustrative, not unit identities')
    ...
    manifest['dredge_source']='luke_early_medicine_peak_comparison_v2: reused unchanged fresh fits'
