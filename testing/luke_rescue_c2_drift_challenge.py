...
def prepare_template(full_384: np.ndarray) -> tuple[np.ndarray, int]:
    ...
    taper = np.ones(a.shape[0], dtype=np.float32)
    taper[:guard], taper[-guard:] = ramp, ramp[::-1]
    a = a * taper[:, None]
    a[[0, -1]] = 0.0
    return validate_template(a.astype(np.float32), edge_guard_samples=1), int(peak_col)
...
def run(templates: list[str], out_root: Path | None = None) -> dict:
    _freeze_prespec()
    ...
    for tid in templates:
        ...
        for traj_name in PRESPEC["trajectories"]:
            ...
            rows.append({
                "template": tid, "trajectory": f"PENALTY:{traj_name}",
    ...
    arms = df[~df.trajectory.str.startswith("PENALTY")]
    best_static = arms[arms.trajectory == "static"].sort_values("accuracy").iloc[-1]
    summary = {
        "probe": PRESPEC["probe"],
        "templates": templates,
        ...
        "penalties": {
            f"{r.template}:{r.trajectory.split(':')[1]}": {
            ...
            for r in df[df.trajectory.str.startswith("PENALTY")].itertuples()
        },
    }
