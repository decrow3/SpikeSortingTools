...
OUT=SRC/'luke_early_medicine_peak_comparison_v1'
PRE=SRC/'luke_long_context_validation_v1'
FS=29999.835983263598
ARGS=dict(time_bin_size=1.,num_depth_bins=2,time_kernel_width=50.,amplitude_threshold_quantile=0.,training_steps=10000,plot_figures=False)
def main():
    ...
    for col,arm in enumerate(['original','compensated']):
        z=np.load(PRE/f'{arm}_dredge_motion.npz')
        methods=[('DREDGE',z['time_s'],z['depth_um'],z['displacement_um'],'#56E0E8'),('MEDiCINe',np.load(OUT/arm/'time_bins.npy'),np.load(OUT/arm/'depth_bins.npy'),np.load(OUT/arm/'motion.npy'),'#FFFFFF')]
        for row,(lo,hi,anchors) in enumerate(rows):
            ...
            for name,t,y,d,color in methods:
                ...
                for anchor in anchors:
                    ...
                    ax.plot(t,anchor+trace,color=color,lw=1.4,label=name if anchor==anchors[0] else None)
