...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';OUT=SRC/'luke_early_medicine_peak_comparison_v4';OLD=SRC/'luke_early_medicine_peak_comparison_v1'
...
for col,arm in enumerate(['original','compensated']):
 ...
 for name,src,c in [('MEDiCINe 1 s · 2 bins',SRC/'luke_early_medicine_peak_comparison_v2','#D55E00'),('MEDiCINe 1 s · 4 bins',OUT,'#009E73')]:
...
fig.suptitle('MEDiCINe 1 s kernel: 2 versus 4 depth bins · same peaks · full displacement range\nAll traces referenced to their median at 970–975 s; shared vertical scale within each row')
