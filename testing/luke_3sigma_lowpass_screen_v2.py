...
import json,hashlib,time,os,sys,traceback
...
SRC=ROOT/'testing/outputs';PRE=SRC/'luke_detection_threshold_sweep_v1';OUT=SRC/'luke_3sigma_lowpass_screen_v2'
...
def save_added_examples(broad,narrow,bp,det,geo,fs,start):
 # Exact channel/sample novelty is descriptive: low-pass timing shifts can create it.
 rows=[];waveforms={}
 fig,axes=plt.subplots(4,2,figsize=(12,11),layout='constrained')
 for col,(name,p) in enumerate(det.items()):
  added=~np.isin(keys(p),keys(bp))
  for band in range(4):
   ax=axes[band,col];ids=np.flatnonzero(added&(geo[p['channel_index'],1]>=band*960)&(geo[p['channel_index'],1]<(band+1)*960))
   if not len(ids):ax.text(.5,.5,'No unmatched detections',ha='center',transform=ax.transAxes);continue
   j=ids[len(ids)//2];ch=int(p['channel_index'][j]);sample=int(p['sample_index'][j]);channels=np.flatnonzero(abs(geo[:,1]-geo[ch,1])<=60);off=np.arange(-30,31)
   wb=broad[sample+off[:,None],channels[None,:]];wn=narrow[sample+off[:,None],channels[None,:]];ci=np.flatnonzero(channels==ch)[0]
   ax.plot(off/fs*1000,wn,color='gray',alpha=.2);ax.plot(off/fs*1000,wb[:,ci],label='Broad detector');ax.plot(off/fs*1000,wn[:,ci],label='Low-pass detector');ax.set(title=f'{name} · ch{ch} · {start+sample/fs:.4f}s',xlabel='Time (ms)',ylabel='µV');ax.legend(fontsize=6)
   key=f'{name}_{band}';waveforms[key+'_broad_uv']=wb;waveforms[key+'_lowpass_uv']=wn;waveforms[key+'_channels']=channels;rows.append(dict(arm=name,depth_band=band,channel=ch,sample_index=sample,time_s=start+sample/fs))
 fig.suptitle('Detections absent at the exact baseline channel/sample\nTiming shifts may account for apparent novelty; examples selected without motion scores')
 for ext in ['png','pdf']:fig.savefig(OUT/f'05_added_detection_examples.{ext}',dpi=140)
 plt.close(fig);pd.DataFrame(rows).to_csv(OUT/'added_detection_examples.csv',index=False);np.savez_compressed(OUT/'added_detection_waveforms.npz',**waveforms)

def main():
 run_started=time.monotonic();timings=[]
 ...
 settings=dict(interval_s=[4160,4260],arms=ARMS,screen='Detector-waveform energy within ±0.5ms / energy within ±1ms >=0.65; no additional amplitude or neighbor gate',lowpass='Third-order Butterworth3000Hz forward/backward AFTER existing broadband300–6000Hz filtering and frozen shared-response compensation; 50ms external padding; no change to model fit or sorting voltage',noise_control='fixed retains baseline noise vector; adjusted multiplies it by per-channel MAD(lowpass)/MAD(broad compensated) measured once on4180–4200s and then frozen',calibration_interval_s=[4180,4200],detection=dict(method='locally_exclusive',peak_sign='neg',radius_um=50,detect_threshold=3),localization=dict(method='monopolar_triangulation',radius_um=75,n_jobs=4),reuse='Broadband3sigma peaks/locations exactly reused; narrowband detections all freshly localized on narrowband voltage. Shared narrowband events localized once.',estimator=cfg,strict_bound_um=80,comparison='Same nine event-matched lighthouse tracks, overall and drop/recovery scores; previously inspected interval, no independent validation',resume='Chunk arrays/masks/fields persist; no automatic resume or within-stage checkpoint; no sort',model_sha256=hashlib.sha256(modelpath.read_bytes()).hexdigest(),baseline_noise_sha256=hashlib.sha256(noisepath.read_bytes()).hexdigest())
 settings['source_sha256']={str(path.relative_to(ROOT)):file_digest(path) for path in [Path(__file__).resolve(),ROOT/'testing/luke_dredge_bounded.py',ROOT/'testing/luke_screen_sweep.py',ROOT/'testing/luke_peak_population_review.py']}
 settings['baseline_sources_sha256']={str(path):file_digest(path) for path in sorted(PRE.glob('*3sigma*.npy'))+[PRE/'fields/3sigma.npz',PRE/'settings.json',BASE/'recording/rescue_recording_manifest.json']}
 settings['thread_environment']={key:os.environ.get(key) for key in ['OMP_NUM_THREADS','MKL_NUM_THREADS','OPENBLAS_NUM_THREADS','NUMEXPR_NUM_THREADS']}
 settings['resume']='No automatic resume or within-stage checkpoint. Existing output directory is refused; failures must be investigated before a new version or audited recovery. Persisted arrays alone are not certified reusable stages.'
 ...
 for start in [4180,4160,4200,4220,4240]:
  ...
  joined=np.concatenate(list(det.values()));uk,ii=np.unique(keys(joined),return_index=True);union=joined[ii];print(start,'localizing narrowband union',len(union),flush=True);loc=localize_peaks(record,union,method='monopolar_triangulation',radius_um=75.,n_jobs=4,mp_context='fork',chunk_duration='1s',progress_bar=False);assert np.isfinite(loc['y']).all();timings.append(dict(stage='union_localization',start_s=start,seconds=time.monotonic()-localization_started,peaks=len(union)));chunk={'broad_3sigma':(bp,by),'broad_screen':(bp[bk],by[bk])}
  ...
  if start==4180:
   save_added_examples(broad,narrow,bp,det,geo,fs,start)
  timings.append(dict(stage='chunk_complete',start_s=start,seconds=time.monotonic()-chunk_started));pd.DataFrame(timings).to_csv(OUT/'timings.csv',index=False)
 ...
 for name in ARMS:
  ...
  if name=='broad_3sigma':
   oldp=np.load(PRE/'3sigma_peaks.npy');oldy=np.load(PRE/'3sigma_locations.npy');assert np.array_equal(p,oldp) and np.array_equal(y,oldy);old=np.load(PRE/'fields/3sigma.npz');np.savez_compressed(OUT/'fields'/f'{name}.npz',**{key:old[key] for key in old.files})
  else:
  ...
  timings.append(dict(stage='motion_field',arm=name,seconds=time.monotonic()-field_started));pd.DataFrame(timings).to_csv(OUT/'timings.csv',index=False)
 ...
 d=pd.read_csv(OUT/'event_matched_predictions.csv');tracks=pd.read_csv(SRC/'luke_lighthouse_gentle_v1/gentle_tracks.csv');colors=['#2878b5','#b13775','#37906c','#d49a00','#7b52ab','#333333'];mainarms=ARMS;fig,axs=plt.subplots(3,3,figsize=(16,11),sharex=True,layout='constrained')
 ...
 for label in ['broad','lowpass_adjusted']:
  ...
  for band,ax in enumerate(axs):
   for a,retain in zip(ax,[False,True]):
    if (label,band,retain) not in examples:
     a.text(.5,.5,'No events in this category',ha='center',va='center',transform=a.transAxes);a.set_axis_off();continue
 ...
 timings.append(dict(stage='total',seconds=time.monotonic()-run_started));pd.DataFrame(timings).to_csv(OUT/'timings.csv',index=False)
