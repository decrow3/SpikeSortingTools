...
def main():
    ...
    cfg=json.loads((PRE/'settings.json').read_text())['estimator'];cfg.update(bin_s=.25,max_disp_um=250.,histogram_time_smooth_s=1.)
    (OUT/'dredge_settings.json').write_text(json.dumps(dict(estimator=cfg,strict_pairwise_bound_um=250,scope='Fresh fit on exact 930–1030 cached peaks, 0.25 s bins, 1 s Gaussian histogram smoothing, ±250 µm strict pairwise bound',source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()),indent=2))
