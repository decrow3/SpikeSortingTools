...
ROOT=Path(__file__).resolve().parents[1];SRC=ROOT/'testing/outputs';OUT=SRC/'luke_early_medicine_peak_comparison_v8'
def main():
    ...
    for col,arm in enumerate(['original','compensated']):
        ...
        methods=[('DREDGE · 0.5 s Gaussian',z['time_s'],z['depth_um'],z['displacement_um'],'#56E0E8'),('MEDiCINe · 1 s kernel · 0.25 s time bins · 4 depth bins',np.load(SRC/'luke_early_medicine_peak_comparison_v6'/arm/'time_bins.npy'),np.load(SRC/'luke_early_medicine_peak_comparison_v6'/arm/'depth_bins.npy'),np.load(SRC/'luke_early_medicine_peak_comparison_v6'/arm/'motion.npy'),'#FFFFFF')]
