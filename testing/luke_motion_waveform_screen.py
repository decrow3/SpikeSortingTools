...
def main():
 ...
 for ch in range(384):
  ...
  passed=gate&(best>=.85);keep[ids]=passed
  for j,idx in enumerate(ids):
   reason='retained' if passed[j] else 'shared_explained' if shared[j] else 'low_snr' if snr[j]<8 else 'offcenter_or_complex' if concentration[j]<.65 else 'weak_spatial_support' if not support[j] else 'no_repeating_family'
   rows.append(dict(peak_index=int(idx),time_s=float(4180+ev[j]/fs),channel=ch,depth_um=float(y['y'][idx]),half=int(ev[j]>=10*fs),snr=float(snr[j]),central_energy_fraction=float(concentration[j]),shared_explained=float(explained[j]),shared_cosine=float(sharedcos[j]),family_cosine=float(best[j]),family=int(label[j]),decision=reason,keep=bool(passed[j])))
