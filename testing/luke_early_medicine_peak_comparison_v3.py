...
OUT=SRC/'luke_early_medicine_peak_comparison_v3'
PRE=SRC/'luke_long_context_validation_v1'
FS=29999.835983263598
ARGS=dict(time_bin_size=1.,num_depth_bins=2,time_kernel_width=5.,amplitude_threshold_quantile=0.,training_steps=10000,plot_figures=False)
def main():
    ...
    for col,arm in enumerate(['original','compensated']):
        ...
        methods=[('DREDGE fresh fit',z['time_s'],z['depth_um'],z['displacement_um'],'#56E0E8'),('MEDiCINe · 5 s kernel',np.load(OUT/arm/'time_bins.npy'),np.load(OUT/arm/'depth_bins.npy'),np.load(OUT/arm/'motion.npy'),'#FFFFFF')]
